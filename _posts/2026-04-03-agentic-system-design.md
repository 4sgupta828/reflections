...
A useful lens for calibrating the loop: distinguish capability gaps (the system cannot solve this class of problem at all, regardless of attempts) from reliability gaps (the system can solve it, but not consistently). Capability gaps call for better examples, stronger retrieval, or a more capable model. Reliability gaps call for tighter guardrails, more constrained routing, or a critic step — the raw ability is already there but the policy is not deploying it dependably.
...
If the agent succeeds 3 times out of 10, the model has the capability but is inconsistent — better prompts, few-shot examples, or a critic will help. If it fails every time regardless of how the input is phrased, it is a hard failure — the tool is missing, the context was never retrieved, or the data is stale. Prompt engineering a hard failure wastes time and misleads you about what the real problem is.
...
A prompt edit changes the policy. Run it against the regression suite and compare success rates — including on edge cases — before and after. Silent prompt edits are one of the most common sources of gradual performance degradation, because the change is invisible in deployment but visible only later in outcome metrics.
...
The evolution stages are not abstract — they map directly to how real failure modes get discovered, diagnosed, and resolved. The table below traces six production failure patterns across domains, showing how each stage of system maturity changes what you can see, what you can measure, and how the policy adapts.
